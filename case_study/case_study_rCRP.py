...
class Model:
	...
	def get_topic_for_table(self, table):	
		...
		for i in range(1, self.depth_limit):
			...
			for word in table.words:
				roulette[-2] *= (parent_topic.phi[word] * self.num_word / np.sum(parent_topic.phi) )
...
if __name__ == "__main__":
    ...
    save_zipped_pickle(model, exp_dir + param + '/rCRP_model.p')    

    '''
    # or load existing model 
    with gzip.open(exp_dir + param + '/rCRP_model.p', 'rb') as f:
    ...
    sheet1.write(0,0,'rCRP') 
    for i in range(len(avg_scores)):
        sheet1.write(i+1,0,avg_scores[i]) 
    f.save('restaurant_test_score_rCRP.xls')
